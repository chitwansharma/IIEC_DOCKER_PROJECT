...
1. In redhat enterprise linux install the docker software.
2. set up the things-- *disabling firewall* becoz it might block some metworking stuffs.
                       *starting the docker* (command - systemctl start docker)
3. download reqiured images -- *mysql image*
                               *jhoomla image*(by apply pull command)
4. setting up mysql --     use command --- docker run -it -e MYSQL_ROOT_PASSWORD=(any password you like) -e MYSQL_USER=(any user name) -e MYSQL_PASSWORD=(any password(recommended not to use root password) -e MYSQL_DATABASE=(any database name) --name joomladb mysql:5.6
                   --- then use command -- yum install mysql
# docker compose
5.before using docer compose you have to use the software *dockercompose*
  then you can create and edit file using command-- vim docker-compose.yml
   write all the command in the file and save it
	...
	# jhoomla started
	 7. open google and type --localhist:80 
	...
	# Docker container start stop
	 ...
	 #docker compose down:
	 you can easily stop the container using command --docker compose down
